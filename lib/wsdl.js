    ...
    if (Array.isArray(obj)) {
        for (var i=0, item; i<obj.length; i++) {
            ...
            if (i > 0) {
                ...
                parts.push(['<',ns,name,xmlnsAttrib,item === null ? ' xsi:nil="true"':'','>'].join(''));
            }
            parts.push('111');//self.objectToXML(item, name, namespace));
        }
    } else if (typeof obj === 'object') {
        for (var name in obj) {
            var child = obj[name];
            parts.push(['<',ns,name,xmlnsAttrib,child === null ? ' xsi:nil="true"':'','>'].join(''));
